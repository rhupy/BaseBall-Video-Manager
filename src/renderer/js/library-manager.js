...
class LibraryManager {
  ...
  initModal() {
    ...
    document
      .getElementById("open-data-folder")
  ...
  selectLibrary(index) {
    // 이전 선택 해제
    const prevSelected = this.listContainer.querySelector(
    ...
    const newSelected = this.listContainer.querySelector(
      `[data-index="${index}"]`
    );
    if (newSelected) {
  ...
  async openDataFolder() {
    try {
      await window.electronAPI.invoke("open-folder", this.dataPath);
      Utils.updateStatus("데이터 폴더를 열었습니다.");
    } catch (error) {
      console.error("데이터 폴더 열기 실패:", error);
      alert("데이터 폴더를 열 수 없습니다.");
    }
  }
