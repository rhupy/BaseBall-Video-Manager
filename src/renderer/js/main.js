...
class App {
  ...
  async init() {
    try {
      ...
      this.showErrorMessage('애플리케이션 초기화에 실패했습니다: ' + error.message);
    }
  }
  ...
  async initManagers() {
    ...
    this.virtualScroll = new VirtualScroll(scrollContainer, CONSTANTS.VIRTUAL_ITEM_HEIGHT);
  ...
  switchTab(tabName) {
    ...
    Utils.updateStatus(`${tabName === 'video' ? '비디오' : '기타'} 파일 모드로 전환`);
  }
  ...
  getAppState() {
    return {
      ...
      fileCount: this.fileManager ? this.fileManager.getCurrentFiles().length : 0,
      libraryCount: this.libraryManager ? this.libraryManager.getLibraries().length : 0,
      searchQuery: this.searchEngine ? this.searchEngine.searchInput?.value || '' : ''
    };
  }
