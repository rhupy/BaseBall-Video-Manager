...
class FileManager {
  ...
  buildSearchIndex() {
    ...
    const buildIndexForType = (files, type) => {
      files.forEach((file, index) => {
        const words = file.Filename.toLowerCase().split(/\W+/).filter(word => word.length > 0);
        words.forEach(word => {
  ...
  search(query) {
    ...
    if (searchWords.length > 0) {
      ...
      if (this.searchIndex.has(firstWord)) {
        const indices = this.searchIndex.get(firstWord);
        indices.video.forEach(i => results.video.add(i));
        indices.file.forEach(i => results.file.add(i));
      }
    }
    ...
    for (let i = 1; i < searchWords.length; i++) {
      ...
      if (this.searchIndex.has(word)) {
        ...
        // 교집합만 남김
        results.video = new Set([...results.video].filter(idx => indices.video.includes(idx)));
        results.file = new Set([...results.file].filter(idx => indices.file.includes(idx)));
      } else {
        // 단어가 인덱스에 없으면 결과 없음
        results.video.clear();
        results.file.clear();
    ...
    // 결과를 실제 파일 객체로 변환
    this.filteredFiles.video = [...results.video].map(i => this.allFiles.video[i]);
    this.filteredFiles.file = [...results.file].map(i => this.allFiles.file[i]);
  ...
  async refreshFiles(isAutoSync = false) {
    ...
    this.isLoading = true;
    const statusText = isAutoSync ? '자동 동기화 중...' : '파일을 새로고침하는 중...';
    Utils.updateStatus(statusText);

    try {
      // 라이브러리 경로들 로드
      const libraryResult = await window.electronAPI.invoke('load-json-file', `${this.dataPath}/lib.json`);
      ...
      for (const type of ['video', 'file']) {
        ...
        for (const library of libraries) {
          Utils.updateProgress(processedPaths, totalPaths, `${library.path} 스캔 중...`);

          // 라이브러리 접근 가능 여부 확인
          const libraryExists = await window.electronAPI.invoke('path-exists', library.path);
          if (!libraryExists) {
          ...
          const scanResult = await window.electronAPI.invoke('scan-directory', library.path, extensions[type]);
          ...
          if (scanResult.success) {
            scanResult.files.forEach(scannedFile => {
              ...
              // 기존 파일 찾기
              const existingFile = existingFiles.find(f => f.Fullpath === scannedFile.fullpath);
        ...
        // 추가 시간 역순으로 정렬
        this.allFiles[type].sort((a, b) => new Date(b.Addtime) - new Date(a.Addtime));

        // JSON 파일 저장
        const fileName = type === 'video' ? 'media/files.json' : 'file/files.json';
        const filePath = `${this.dataPath}/${fileName}`;
        await window.electronAPI.invoke('save-json-file', filePath, this.allFiles[type]);
      }
      ...
      if (inaccessibleLibraries.length > 0) {
        ...
        if (!isAutoSync) { // 수동 새로고침시에만 알림 표시
          alert(
      ...
      Utils.updateStatus(`${completedText} - 비디오: ${this.allFiles.video.length}개, 기타: ${this.allFiles.file.length}개`);
  ...
  updateLastAccessTime(filePath) {
    ...
    for (const type of ['video', 'file']) {
      ...
      if (file) {
        ...
        // UI에도 반영
        const filteredFile = this.filteredFiles[type].find(f => f.Fullpath === filePath);
        if (filteredFile) {
  ...
  updateUI() {
    ...
    // 확장자 목록 업데이트
    const extensions = this.currentTab === 'video' ? CONSTANTS.EXTENSIONS.VIDEO : CONSTANTS.EXTENSIONS.FILE;
  ...
  async removeEmptyFolders(libraryPath) {
    try {
      const result = await window.electronAPI.invoke('remove-empty-folders', libraryPath);
      if (result.success && result.count > 0) {
        console.log(`${libraryPath}에서 ${result.count}개의 빈 폴더를 제거했습니다.`);
      }
    } catch (error) {
  ...
  async removeFileFromList(filePath) {
    for (const type of ['video', 'file']) {
      // 전체 목록에서 제거
      const allIndex = this.allFiles[type].findIndex(f => f.Fullpath === filePath);
      if (allIndex !== -1) {
      ...
      // 필터링된 목록에서도 제거
      const filteredIndex = this.filteredFiles[type].findIndex(f => f.Fullpath === filePath);
      if (filteredIndex !== -1) {
